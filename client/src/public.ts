...
export function publicHeader(signedIn = false) {
  return `<header class="public-header">${wordmark()}<nav class="marketing-nav" id="marketing-nav" aria-label="Navigation du site"><a href="#welcome/methode">La méthode</a><a href="#welcome/benefices">Votre préparation</a><a href="#pricing"><md-ripple></md-ripple>Tarif et accès</a><a href="#welcome/faq">Questions fréquentes</a><a class="mobile-account-link" href="#${signedIn ? "home" : "login"}">${signedIn ? "Mon espace" : "Se connecter"}</a></nav><div class="public-header-actions">${signedIn ? button("Mon espace", "go-home", "filled", "arrow_forward") : `<a href="#login" class="login-link">Se connecter</a>${button("Commencer", "register")}`}<md-icon-button class="public-menu-button" data-act="public-menu" aria-controls="marketing-nav" aria-expanded="false" aria-label="Ouvrir la navigation">${icon("menu")}</md-icon-button></div></header>`;
}
export function publicFooter() {
  return `<footer class="public-footer"><div>${wordmark()}<p>Préparez vos réponses.<br>Présentez votre expérience.</p></div><nav aria-label="Liens de bas de page"><a href="#welcome/methode">Comment ça marche</a><a href="#welcome/faq">Questions fréquentes</a><button data-act="privacy">Confidentialité</button></nav><div class="footer-signature"><span>Interview Prep AI</span><span>Préparation aux entretiens</span></div></footer>`;
}
export function landing(signedIn = false) {
  const start = signedIn ? "go-home" : "register";
  return `<div class="public-site">${publicHeader(signedIn)}<main id="main-content"><section class="landing-hero public-width"><div class="landing-copy"><div class="eyebrow"><span class="edition-dot"></span> LE PROCHAIN ENTRETIEN SE PRÉPARE</div><h1>Vous avez<br>l’expérience.<br><em>Trouvez les mots.<svg class="headline-stroke" viewBox="0 0 340 20" fill="none" preserveAspectRatio="none" aria-hidden="true"><path pathLength="1" d="M7 14Q164 1 332 11"/></svg></em></h1><p class="landing-intro">Un CV ne raconte pas tout. Préparez-vous à expliquer vos choix, défendre vos compétences et parler de vos réalisations.</p><div class="landing-cta">${button("Préparer mon entretien", start, "filled", "arrow_forward")}<a class="method-link" href="#welcome/methode">Découvrir la méthode ${icon("south")}</a></div><div class="landing-facts"><span>${icon("description")} À partir de votre CV</span><span>${icon("pace")} Sans chronomètre</span></div></div><div class="landing-visual"><div class="visual-kicker"><span>LE POSTE QUE VOUS VISEZ.</span><span>LE PARCOURS QUI EST LE VÔTRE.</span></div><div class="landing-illustration">${heroArt()}</div><div class="scene-note scene-note-a" aria-hidden="true">${icon("chat_bubble")} Votre parcours</div><div class="scene-note scene-note-b" aria-hidden="true">${icon("check_circle")} Des exemples concrets</div><div class="visual-caption"><span class="caption-line"></span><p>Au-delà du CV,<br><strong>il y a votre façon d’en parler.</strong></p></div><div class="visual-stamp" aria-hidden="true">${brandMark()}</div></div></section><div class="journey-strip public-width" aria-label="Les trois étapes de votre préparation"><span><b>01</b> Votre CV</span>${icon("arrow_right_alt")}<span><b>02</b> Vos réponses</span>${icon("arrow_right_alt")}<span><b>03</b> Votre bilan</span></div><section id="methode" tabindex="-1" class="landing-section public-width"><div class="landing-section-head"><div><div class="eyebrow">LA MÉTHODE</div><h2>Un entretien ne s’improvise pas.<br>Sa préparation peut rester simple.</h2></div><p>De votre CV au bilan, tout se passe dans le même espace. Vous choisissez le poste, le niveau et la longueur de l’entretien.</p></div><div class="method-grid">${[
    [
      "01",
